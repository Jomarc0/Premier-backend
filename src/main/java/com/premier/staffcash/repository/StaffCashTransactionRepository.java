...
public interface StaffCashTransactionRepository extends JpaRepository<StaffCashTransaction, Long> {
    ...
    @org.springframework.data.jpa.repository.Query("select t from StaffCashTransaction t join fetch t.staff join fetch t.vehicle join fetch t.driverShift where t.staff.id = :staffId and coalesce(t.offlineCapturedAt,t.createdAt) >= :from and coalesce(t.offlineCapturedAt,t.createdAt) < :to order by t.createdAt desc")
    ...
    @org.springframework.data.jpa.repository.Query("select t from StaffCashTransaction t join fetch t.staff join fetch t.vehicle join fetch t.driverShift where coalesce(t.offlineCapturedAt,t.createdAt) >= :from and coalesce(t.offlineCapturedAt,t.createdAt) < :to order by t.createdAt desc")
